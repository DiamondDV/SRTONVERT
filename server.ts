    ...
    if (payload && supabase) {
      ...
      if (profileError) {
        ...
        if (profileError.code === '42P01') {
          console.error("HINT: The 'profiles' table does not exist. Please run the provided SQL in your Supabase SQL Editor.");
        }
      }
      ...
      if (!sub) {
        ...
        if (insertError) {
          ...
          if (insertError.code === '42P01') {
            console.error("HINT: The 'subscriptions' table does not exist. Please run the provided SQL in your Supabase SQL Editor.");
          }
        }
      }
    }
...
app.get("/api/subscription", async (req, res) => {
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42P01') {
        console.error("CRITICAL: The 'subscriptions' table does not exist in your Supabase database.");
        console.error("ACTION REQUIRED: Please copy the SQL script from the warning banner in the web app and run it in your Supabase SQL Editor.");
      }
      return res.json({ subscription: { plan_id: 'free', status: 'active' } });
    }
